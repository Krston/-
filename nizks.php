<?php
//=========التوكن=========//
$token = "5716828192:AAGyzWCcwpowSQ56LPXEVDLiCqFzGdZ_mRA"; 
...
#############
$admin = "5786462868";
$Dev = array("$admin","5786462868");
$eri = array("$admin","5786462868");
$idbot = "5716828192";
$sudo = "$admin";
$buy = "krsronn";
$DevUser = "@$buy";
$devchink = "Krston";
$userbot = "Loviabkrston_bot";
$chsource = "$devchink";
############
if ($tc == 'private'){  
@$user = json_decode(file_get_contents("data/user.json"),true);
...
$IDKrston = str_replace('رفع ادمن ','',$text);
if ($text =="رفع ادمن $IDKrston" and preg_match('/([0-9])/i',$IDKrston) and !in_array($IDKrston,$admin_user)){
$IDKrston= str_replace('رفع ادمن ','',$text);
$KrstonID = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDKrston"))->result->username;
$KrstonNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDKrston"))->result->first_name;
	if($settings["lock"]["rfaabot"] == "مقفول"){
			file_put_contents("data/admin_user/$chat_id.txt",$IDKrston . "\n" , FILE_APPEND);
			file_put_contents("data/admin_user/$chat_id/mange.txt" , " *𓆩* [" . "@". $KrstonID ."] *𓆪* " . "»" . " *𓆩* `". $IDKrston ."` *𓆪* ". "\n" , FILE_APPEND);
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
⋄︙ العضو ⋙「 [$KrstonNAME](tg://user?id=$IDKrston) 」 
⋄︙ تم ترقية ليصبح ادمن
",'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
]);
}
...
if ($text =="رفع ادمن $IDKrston" and preg_match('/([0-9])/i',$IDKrston) and in_array($IDKrston,$admin_user)){
$IDKrston= str_replace('رفع ادمن ','',$text);
$KrstonID = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDKLOS"))->result->username;
$KrstonNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDKLOS"))->result->first_name;
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
⋄︙ العضو ⋙「 [$KrstonNAME](tg://user?id=$IDKrston) 」 
⋄︙ هوه بالفعل ادمن
...
$IDKrston = str_replace('تنزيل ادمن ','',$text);
if ($text =="تنزيل ادمن $IDKrston" and preg_match('/([0-9])/i',$IDKrston) and in_array($IDKrston,$admin_user)){
$IDKrston= str_replace('تنزيل ادمن ','',$text);
$KrstonID = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDKrston"))->result->username;
$KrstonNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDKrston"))->result->first_name;
	$IDKrston_info = file_get_contents("data/admin_user/$chat_id.txt");
	$admn = file_get_contents("data/admin_user/$chat_id/mange.txt");
	$admn1 = explode("             \n",$admn);
	$str = str_replace($IDKrston,"",$IDKrston_info);
	$str2 = str_replace(" *𓆩* [" . "@". $KrstonID ."] *𓆪* " . "»" . " *𓆩* `". $IDKrston ."` *𓆪* ","",$admn1);
...
$HAMD = file_get_contents("data/Krston.json");
...
file_put_contents("data/Krston.json","CARLOS");
}}
...
file_put_contents("data/namebot.txt","لوفيا");
...
$abuehab = array("هلا قلبي معاك لوفيا","تفضل حبيبي انا لوفيا🌝💞","يمك حياتي امرني 🥺💞","شتريد كل شويه لوفيا","يابه نجب كل شويه لوفيا خبصتنه");
$ehab = array_rand($proxre, 1);
if($text == "لوفيا" and $namebot == NULL){
if ($tc == 'group' | $tc == 'supergroup'){
...
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"⋄︙اوامر الحمايه لسورس اللوفيا
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
⋄︙قفل ، فتح ← الامر 
...
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"⋄︙اوامر الاعضاء لسورس اللوفيا
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
...
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
⋄︙قناة البوت ← [@$devchink]";
$m6 = "⋄︙اوامر الاعضاء لسورس اللوفيا
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
⋄︙تحويل/بالرد - لتحويل صيغ 
⋄︙ملصق ← صوره
⋄︙صوره ← ملصق
...
$k = file_get_contents("https://ali-apii.ml/api/zhrfa_Krston.php?text=".$text);
bot('sendMessage',[
 'chat_id'=>$chat_id,
'text'=>"$k
 ـ────────────
...
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text'=>"⋄︙قائمة ملفات متجر سورس اللوفيا
...
ᯓKrston
 ...
 ⌯- لتنصيب سورس اللوفيا الجديد v2.0.0 ⇓⇓⇓ 

`cd ; git clone https://github.com/kiindi/Krston.git; cd Krston ; chmod +x ins.py ; python3 ins.py`
...
########
if($text == "⋄ تجربة السورس"){if(in_array($from_id,$Dev) or in_array($from_id,$eri)) {bot("sendmessage",['chat_id'=>$chat_id,'text'=>"✵⁞ *اليك قائمة تجربة السورس*\n━━━━━━━━━━━━━\n⌯⁞ [كروب تجربة السورس](https://t.me/Krstonblovia)\n⌯⁞ [قناة شروحات السورس](https://t.me/TH3SS)\n━━━━━━━━━━━━━\n⌯⁞ [مطور السورس](https://t.me/krstonn)",'parse_mode'=>"Markdown",'disable_web_page_preview'=> true ,'reply_to_message_id'=>$message_id,'reply_markup'=>json_encode(['inline_keyboard'=>[[['text'=>"⌯⁞ كروب تجربة السورس ⁞⌯",'url'=>"t.me/Krstonblovia"]],[['text'=>"⌯⁞ مطور السورس ⁞⌯",'url'=>"t.me/$buy"]],]])]);}}
#######
...
⋄︙تغير الايدي ↫ تغ
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
[Source Krston](https://t.me/kitabatkrston)",
'parse_mode'=>"markdown",'reply_to_message_id'=>$message_id,
 ]);
 } 
 #---------------#
